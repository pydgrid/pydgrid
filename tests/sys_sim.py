...
class system(object):
    ...
    def __init__(self,json_file=None):
        ...
        dt_list += [('u',np.float64,(3,1))]
        struct_list += [np.zeros((4,1))]
        struct_list += [np.zeros((9,1))]
        struct_list += [np.zeros((3,1))]
...
def update(struct,call,item):
    ...
    P_t = struct[item]['y'][5,0]
    Q_t = struct[item]['y'][6,0]
    V_t = struct[item]['y'][7,0]
    theta_t = struct[item]['y'][8,0]

    p_m = struct[item]['u'][0,0]
    v_f = struct[item]['u'][1,0]
    D = struct[item]['u'][2,0]
    ...
    struct[item]['G_y'][3,7] = -np.sin(delta - theta_t) 
    struct[item]['G_y'][3,8] = V_t*np.cos(delta - theta_t) 
    struct[item]['G_y'][4,4] = 1 
    struct[item]['G_y'][4,7] = -np.cos(delta - theta_t) 
    struct[item]['G_y'][4,8] = -V_t*np.sin(delta - theta_t) 
    struct[item]['G_y'][5,0] = v_d 
    struct[item]['G_y'][5,1] = v_q 
    struct[item]['G_y'][5,3] = i_d 
    struct[item]['G_y'][5,4] = i_q 
    struct[item]['G_y'][5,5] = -1 
    struct[item]['G_y'][6,0] = v_q 
    struct[item]['G_y'][6,1] = -v_d 
    struct[item]['G_y'][6,3] = -i_q 
    struct[item]['G_y'][6,4] = i_d 
    struct[item]['G_y'][6,6] = -1 
    struct[item]['G_y'][7,5] = -1 
    struct[item]['G_y'][7,7] = -V_inf*(-B_t_inf*np.sin(theta_inf - theta_t) + G_t_inf*np.cos(theta_inf - theta_t)) + 2*V_t*(G_t0 + G_t_inf) 
    struct[item]['G_y'][7,8] = -V_inf*V_t*(B_t_inf*np.cos(theta_inf - theta_t) + G_t_inf*np.sin(theta_inf - theta_t)) 
    struct[item]['G_y'][8,6] = -1 
    struct[item]['G_y'][8,7] = -V_inf*(-B_t_inf*np.cos(theta_inf - theta_t) - G_t_inf*np.sin(theta_inf - theta_t)) - 2*V_t*(B_t0 + B_t_inf) 
    struct[item]['G_y'][8,8] = -V_inf*V_t*(-B_t_inf*np.sin(theta_inf - theta_t) + G_t_inf*np.cos(theta_inf - theta_t)) 
